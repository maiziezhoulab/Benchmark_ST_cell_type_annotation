...
library(ggplot2)
# conda activate hest
big_csv  <- "/maiziezhou_lab2/yuling/MouseSpinal/Project/GraphST/0503_F4_C_output/big_group_accuracy_per_series.csv"
sub_csv  <- "/maiziezhou_lab2/yuling/MouseSpinal/Project/GraphST/0503_F4_C_output/subgroup_accuracy_per_series.csv"
id_csv   <- "/maiziezhou_lab2/yuling/MouseSpinal/Project/GraphST/0503_F4_C_output/identity_accuracy_per_series.csv"
...
drop_subgroups <- c("Dorsal Maf(1)", "Dorsal Maf(2)")
sub_long_tree <- sub_long %>% filter(!name %in% drop_subgroups)
...
edges_children <- lapply(big_names, function(g) {
  ...
  subs <- sub_long_tree %>% filter(str_detect(name, patt)) %>% pull(name)
...
node_sub  <- sub_long_tree %>% select(name, value)
...
# save as pdf 
pdf("/maiziezhou_lab2/yuling/MouseSpinal/label_transfer/metric/tree_spread_GraphST_17_0503_F4_C_0606.pdf",
...
ggraph(lay) +
  ...
  ggtitle("Accuracy Tree for GraphST") +
